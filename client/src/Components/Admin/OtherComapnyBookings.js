...
function OtherCompanyBookings() {
  ...
  const auth=useContext(AuthContext);
    ...
    useEffect(()=>{
            const getAllDetails=async()=>{
                try{
                    const response=await fetch("http://localhost:5000/api/bookings/bookingdetailByMember",{
                      method: "POST",
                      body: JSON.stringify({
                        member: "OTHER_COMPANY",
                      }),
                      headers: {
  ...
  return (
    ...
    <div>
         ...
         <table style={{marginLeft: "150px"}} className="table">
...
export default OtherCompanyBookings;